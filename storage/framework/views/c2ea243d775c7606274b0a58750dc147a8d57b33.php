<?php $__env->startSection('title', 'Investment Contract'); ?>
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="max-w-4xl mx-auto mb-8 flex justify-between items-center">
        <div>
            <a href="<?php echo e(route('user.plans.details', $userPlan->id)); ?>" class="inline-flex items-center px-4 py-2 bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-white rounded-lg transition-colors duration-300">
    ...
    <div class="contract-container">
        <div class="contract-header">
            <div class="flex justify-center mb-4">
                <img src="<?php echo e(asset('dash/images/logo.png')); ?>" alt="BlueTrade Logo" class="h-16">
            </div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-1">INVESTMENT AGREEMENT</h1>
            <p class="text-gray-500 dark:text-gray-400">Contract #<?php echo e(strtoupper(substr(md5($userPlan->id), 0, 8))); ?></p>
        </div>

        <div class="contract-section">
            <p>This Investment Agreement (the "Agreement") is entered into on <?php echo e($userPlan->created_at->format('F d, Y')); ?> ("Effective Date") by and between:</p>
            ...
            <p>and</p>
            <p><strong><?php echo e(auth()->user()->name); ?></strong>, with registered address at <?php echo e(auth()->user()->address ?: 'Address on file'); ?> ("Investor").</p>
        </div>

        <div class="contract-section">
            <h3>1. INVESTMENT DETAILS</h3>
            <p>The Investor agrees to invest in the Company's <?php echo e($userPlan->plan->name); ?> investment plan with the following terms:</p>

            <table class="contract-table">
                <tr>
                    <td><strong>Investment Plan:</strong></td>
                    <td><?php echo e($userPlan->plan->name); ?></td>
                </tr>
                <tr>
                    <td><strong>Investment Amount:</strong></td>
                    <td><?php echo e(Auth::user()->currency); ?><?php echo e(number_format($userPlan->invested_amount, 2)); ?></td>
                </tr>
                <tr>
                    <td><strong>Return Rate:</strong></td>
                    <td><?php echo e($userPlan->plan->expected_return); ?>% per <?php echo e($userPlan->plan->return_interval); ?></td>
                </tr>
                <tr>
                    <td><strong>Total Expected Return:</strong></td>
                    <td><?php echo e($userPlan->plan->total_return); ?>% (<?php echo e(Auth::user()->currency); ?><?php echo e(number_format($userPlan->expected_profit, 2)); ?>)</td>
                </tr>
                <tr>
                    <td><strong>Investment Term:</strong></td>
                    <td><?php echo e($userPlan->plan->duration); ?> <?php echo e(Str::plural('Day', $userPlan->plan->duration)); ?></td>
                </tr>
                <tr>
                    <td><strong>Start Date:</strong></td>
                    <td><?php echo e($userPlan->activated_at ? $userPlan->activated_at->format('F d, Y') : 'Upon payment confirmation'); ?></td>
                </tr>
                <tr>
                    <td><strong>Maturity Date:</strong></td>
                    <td><?php echo e($userPlan->expires_at ? $userPlan->expires_at->format('F d, Y') : 'To be determined upon activation'); ?></td>
                </tr>
                <tr>
                    <td><strong>Payout Schedule:</strong></td>
                    <td><?php echo e(ucfirst($userPlan->plan->return_interval)); ?></td>
                </tr>
                <?php if($userPlan->compound_interest): ?>
                <tr>
                    <td><strong>Compound Interest:</strong></td>
                    <td>Enabled</td>
                </tr>
                <?php endif; ?>
                <?php if($userPlan->auto_renewal): ?>
                <tr>
        ...
        <div class="contract-section">
            ...
            <p>The funds invested by the Investor will be allocated to <?php echo e(strtolower($userPlan->plan->category->name)); ?> investments as per the Company's investment strategy. The Company will manage these investments on behalf of the Investor to generate the returns specified in this Agreement.</p>
        </div>
        ...
        <div class="contract-section">
            ...
            <p>The risk level for this investment plan is classified as: <?php echo e($userPlan->plan->risk_level <= 2 ? 'Low' : ($userPlan->plan->risk_level <= 3 ? 'Medium' : 'High')); ?></p>
        </div>

        <div class="contract-section">
            <h3>5. EARLY TERMINATION</h3>
            <p>Should the Investor request early termination of this Agreement before the maturity date, an early withdrawal fee of <?php echo e($userPlan->plan->early_withdrawal_fee); ?>% of the invested amount may be applied, and returns may be prorated or forfeited as per the Company's policies.</p>
        </div>
        ...
        <div class="signature-section">
            ...
            <div class="signature-row">
                <div>
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-2">Investor: <?php echo e(auth()->user()->name); ?></p>
                    <p class="text-xs text-gray-500 dark:text-gray-500">Date: <?php echo e($userPlan->created_at->format('F d, Y')); ?></p>
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-500">Date: <?php echo e($userPlan->activated_at ? $userPlan->activated_at->format('F d, Y') : 'Pending activation'); ?></p>
                </div>
            </div>
        ...
        <div class="contract-footer">
            ...
            <p>Document ID: <?php echo e(strtoupper(substr(md5($userPlan->id . auth()->user()->id), 0, 16))); ?></p>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\LENOVO\Herd\vestscapital\resources\views\user\plan\contract.blade.php ENDPATH**/ ?>